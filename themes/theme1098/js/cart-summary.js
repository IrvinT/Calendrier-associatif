...
$(document).ready(function(){
	...
	// $('.cart_quantity_input').click(function(){ console.log('upPPPdate !'); });

	$('.cart_quantity_input').change(function(){
		var id = $(this).attr('name');
		var val = $(this).val();
		updateQty(val, true, id);
	 });
...
function selected()
{
	...
	var val =  $('.cart_quantity.text-center input[type="hidden"]').val(),
		opt = 0;
	
	console.log(val);

	// var opt =  $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector option:contains("'+val+'")');
	
	// var sel =  $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector')
	// $.each(sel, function(i, elmts){

	// 	elm = $(elmts).find('option');
	// 	console.log(elm.val());
	// 	opt = parseInt(elm.val()) == val ? elm : opt ? opt : false;
		
	// });

	// 	if(opt) { opt.prop('selected', true); }
	// console.log('huh ?');
	// $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector option[value="'+val+'"]').prop("selected", true);
	// console.log( $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector option[value="'+val+'"]').val() );
	// console.log( $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector option').val(val) );
	// console.log( $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector').val(val) );
	// $('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector').val(val).prop("selected", true);
	$('.cart_quantity.text-center select.cart_quantity_input.form-control.grey.selector').val(val);
}
...
function updateQty(val, cart, id)
{
	console.log('id : '+id);	// ---------- ---------- ----------	T E S T
	...
	if (!isNaN(parseInt(val)))
	{
		...
		$(prefix + 'input[name=' + id + '_hidden]').prop('val', val);

		if (parseInt(QtyToUp) > 0){
			upQuantity(id.replace('quantity_', ''), QtyToUp, val, hidden);
			console.log('UP : '+QtyToUp);	// ---------- ---------- ----------	T E S T
		}

		else if(parseInt(QtyToUp) < 0){
			downQuantity(id.replace('quantity_', ''), QtyToUp, val, hidden);
			console.log('DOWN : '+QtyToUp);	// ---------- ---------- ----------	T E S T
		}
	}
	else
...
function upQuantity(id, qty, val, hidden)
{
		...
		success: function(jsonData)
		{
			if (jsonData.hasError)
			{
				...
				if (!!$.prototype.fancybox)
				    ...
				    alert(errors);
				$('input[name=quantity_'+ id +']').val(hidden);
				$('input[name=quantity_'+ id +'_hidden]').val(hidden);
			}
			else
			{
				if (jsonData.refresh)
...
function downQuantity(id, qty, val, hidden)
{
	// var val = $('input[name=quantity_' + id + ']').val();

	console.log('id : '+id); // ----------- ----------- ----------- 	T E S T
	console.log('qty : '+qty); // ----------- ----------- ----------- 	T E S T
	console.log('val : '+val); // ----------- ----------- ----------- 	T E S T
