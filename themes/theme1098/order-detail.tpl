...
<div class="box box-small clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<input type="hidden" value="" name="submitReorder"/>
				...
				<a href="#" onclick="$(this).closest('form').submit(); return false;" class="btn btn-default btn-md icon-right pull-right"><span>{l s='Reorder'}</span></a>
			<p class="dark">
...
<div id="order-detail-content" class="table_block table-responsive">
	<table class="table table-bordered">
		...
		{foreach from=$products item=product name=products}
			...
			{/if}
		{/foreach}
		{hook h="displayCartRuleOrderDetail" discounts=$discounts}
...
{if $return_allowed}
	<div id="returnOrderMessage">
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-default btn-sm icon-right"><span>{l s='Make an RMA slip'}</span></button>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="btn btn-default btn-md icon-right"><span>{l s='Send'}</span></button>
		</div>
	</form>
{else}
